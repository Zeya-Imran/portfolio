    ...
    <section className="bento w-11/12 md:w-4/5 mx-auto flex flex-col gap-y-5 overflow-hidden text-white mt-5 md:mt-10">
      ...
      <div className="grid md:grid-cols-3 grid-cols-1 gap-5">
        {experience.map((item, i) => (
          <motion.div
            className="flex flex-col gap-4 resume-ex-box"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
            <div className="flex justify-between ">
              {item.dates} <img src={item.imgUrl} alt={item.id} width={80} />
            </div>
            <Divider
              type="horzontal"
            ...
            <div className="flex justify-between">
              <p className="flex items-center gap-2">
                <FaChalkboardUser style={{ fontSize: "24px" }} /> Role
              </p>{" "}
              <p>Team-collabrator</p>
            </div>
          </motion.div>
        ))}
      </div>
